...
LineState LineTrackingDriver::read() {
    LineState state;
    state.left = digitalRead(PIN_LINE_LEFT) == LOW ? true : false;
    state.middle = digitalRead(PIN_LINE_CENTER) == LOW ? true : false;
    state.right = digitalRead(PIN_LINE_RIGHT) == LOW ? true : false;
    return state;
}